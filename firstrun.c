...
#include "firstrun.h"
/* Design notes.
 * checkfirstrun() returns failure in the event that either the configs
 * dir does not exist or any one of the named config files is absent.
 * firstrun() creates the config dir if needed and copies all config
 * files to it.
 * In the event that firstrun() is run to deal with a missing file, the
 * config dir is cleaned out all all current config files are copied
